...
const Desktop = () => {
    ...
    let afterCardsFetched = (cards) => {
        ...
        for (let i = 0; i < cards.length; i++) {
            document.getElementById('card' + (i + 1)) ? document.getElementById('card' + (i + 1)).firstChild.replaceWith(document.createRange().createContextualFragment(ReactDOMServer.renderToStaticMarkup(cardElements[i]))) : document.body.click()
            gsap.to(cards[i], {
                motionPath: {
                    path: "#path",
                    align: "#path",
